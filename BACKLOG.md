...
| BL-028 | close | architecture | partials.html (shared nav/footer) — COMPLETE. Every page migrated to the shared fetch/inject nav+footer model: pilot index/svc1/blog-003 (PR#36), blog family (PR#45/#46), and the final svc/tool/one-off/hidden families + blog.html sweep (PR#61, merge e13be5d). Fixes delivered: About link restored on blog pages, legacy inline nav on surveys/svy001/inquiry/intake/mtl001 replaced by partials, mobile hamburger nav added sitewide. svy002 excluded by decision. Remaining work: 0. | 2026-07-03 | PR#61 |
...
| BL-034 | open | architecture | Shared stylesheet (STYLE.css) — the migration itself. Approach reframed to phased-from-scratch (not the original big-bang "extract inline CSS sitewide"): STYLE.css v1 (tokens + gutter) authored fresh from STYLE.md and linked sitewide (PR#64 authored, PR#65 linked); the prior 633-line draft was discarded. Remaining: components migrate per-family, each authored fresh from the STYLE.md spec in later phases, replacing pages' inline CSS incrementally. | 2026-07-03 | |
| BL-035 | open | content | SEO / metadata — not yet implemented | 2026-07-03 | |
| BL-036 | close | content | Footer Privacy / Terms placeholders resolved — the two `href="#"` links (Privacy, Terms) collapsed into a single "Privacy & Terms" link → privacy.html in partials.html (propagates sitewide via the shared footer). Merge 721ef54. | 2026-07-03 | PR#62 |
...
| BL-057 | open | architecture | Font-loading ownership → consolidate the Google Fonts `<link>` (DM Serif Display + DM Sans) into the shared partial's `<head>` slice (option B: single-source + performant parallel `<link>`; avoids `@import` serial-load penalty). Deferred to the partials sweep (BL-028); until then pages keep their own font `<link>` and STYLE.css only references the families. Phase 0 evidence: font `<link>`s are NOT uniform — 5 variants (A full+italics ×30, B blog no-italics ×20, C blog-001, D intake, E inquiry); blog posts (B/C) also carry a DUPLICATE font `<link>` buried in the body (~line 328) atop the head one (double-load). Consolidation is a real task. | 2026-07-04 | |
| BL-058 | close | style | STYLE.md line 243 malformed token: form input border written as `#border-dark` (non-existent var, not a hex). Resolved in STYLE.css as #E8E8E8 (`--grey-border`, matching cards). Fix STYLE.md to state #E8E8E8. Also resolve the border-width drift: line 243 says 1.5px, other blocks use 1px — pick one. Done (PR#64): STYLE.md now states 1px #E8E8E8 (`--grey-border`). | 2026-07-04 | PR#64 |
| BL-059 | close | style | STYLE.md Remark callout left rim underspecified ("neutral-dark rim", no hex). Resolved in STYLE.css as #3A3A3A (`--charcoal` — softer neutral-dark, fits the quiet-aside role). Pin #3A3A3A in STYLE.md. | 2026-07-04 | PR#64 |
| BL-060 | close | style | STYLE.md gives no numeric mobile breakpoint; STYLE.css unified at 800px to match the partials nav slice. Document 800px as the governed sitewide breakpoint in STYLE.md so it's canonical, not incidental. | 2026-07-04 | PR#64 |
| BL-061 | open | architecture | BL-028 description correction: repo (main) shows surveys/svy001/inquiry/intake/mtl001 all HAVE full inline nav — BL-028 wrongly states these pages have "no nav." Amend BL-028's own text: these 5 carry legacy inline nav to be *replaced* by partials (like all inline-nav pages), not *added* to. Confirmed by the whole-population superset check. Batch 1a (PR#45) advanced BL-028 (blog family swept, stale about-anchor fixed) but did NOT close it — BL-028 closes only when svc/tool/one-off/hidden families are all swept. UPDATE 2026-07-07: that blocking condition is now MET — the svc/tool/one-off/hidden families + blog.html sweep completed (PR#61), so BL-028 is now closed. | 2026-07-04 | |
...
| BL-067 | open | architecture | Verify whether mailcheck.ai is actually in use. ARCHITECTURE.md lists it as a live dependency (disposable-email screening on tool-gate forms), but current use is unconfirmed. If unused, remove from ARCHITECTURE.md so doc and reality reconverge; if used, add the disclosure back to privacy.html's third-party list (it was removed pending verification). | 2026-07-06 | |
| BL-068 | open | content | privacy.html — counsel review of the whole document before it is relied upon as final. Not yet reviewed by an attorney; the "draft for review" banner was removed for client-facing presentation but legal review still outstanding. | 2026-07-06 | |
| BL-069 | open | content | privacy.html — verify the 18+ age threshold and the parent/guardian carve-out wording with counsel. | 2026-07-06 | |
| BL-070 | open | content | privacy.html — confirm the stated data flow matches reality: survey owners receive respondents' full survey responses. Policy currently states this as fact. | 2026-07-06 | |
| BL-071 | open | content | privacy.html — determine whether New York (or other applicable state) privacy law requires specific consumer-rights disclosures (access/deletion/opt-out language). Current "Your choices" section is generic; a state-specific rights section may be legally required. | 2026-07-06 | |
| BL-072 | open | content | privacy.html — verify New York governing-law clause and whether a venue / dispute-resolution provision should be added, with counsel. | 2026-07-06 | |
| BL-073 | close | style | Scrollbar-gutter reflow (sitewide): no page or partials reserves the vertical scrollbar gutter, so TALL↔SHORT page transitions shift the nav ~15px horizontally on classic-scrollbar setups. Confirmed universal (0 gutter rules across all 55 HTML files). Frequent trigger — the 3 SHORT pages are Contact/Webinars/Livestream (top nav destinations); tool-gate/survey pages flip TALL/SHORT by viewport height (intermittent). Fix: `html { scrollbar-gutter: stable; }`, single additive rule — to land in STYLE.css (Phase 0 of the STYLE.css stand-up). Record as a Section/layout rule in STYLE.md. Done: rule recorded in STYLE.md + implemented in STYLE.css (`html { scrollbar-gutter: stable; }`), linked sitewide (PR#64 authored the rule, PR#65 linked it). Verified working on current browser — the earlier "still shifting" report was stale cache. Current-only browser policy: no `overflow-y: scroll` fallback added. | 2026-07-07 | PR#64 + PR#65 |
| BL-074 | open | process | Variable-naming convention for STYLE.css tokens is recorded in STYLE.md (raw colors `--{family}-{role}`; combos `--combo-XXNN`; type/space/radius scale names). This is the source of truth for token names. Govern and EXTEND it as Phase 1 components migrate and surface names not yet covered — keep STYLE.md the canonical home. | 2026-07-07 | |

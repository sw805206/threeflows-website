...
| BL-073 | open | style | Scrollbar-gutter reflow (sitewide): no page or partials reserves the vertical scrollbar gutter, so TALL↔SHORT page transitions shift the nav ~15px horizontally on classic-scrollbar setups. Confirmed universal (0 gutter rules across all 55 HTML files). Frequent trigger — the 3 SHORT pages are Contact/Webinars/Livestream (top nav destinations); tool-gate/survey pages flip TALL/SHORT by viewport height (intermittent). Fix: `html { scrollbar-gutter: stable; }`, single additive rule — to land in STYLE.css (Phase 0 of the STYLE.css stand-up). Record as a Section/layout rule in STYLE.md. | 2026-07-07 | |
